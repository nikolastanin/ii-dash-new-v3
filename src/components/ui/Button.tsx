...
type Props = Omit<ButtonHTMLAttributes<HTMLButtonElement>, "children" | "onClick"> & {
  ...
  outline?: boolean;
  /** Renders an <a> instead of a <button> — for navigational CTAs (e.g. a tool/calculator link). */
  href?: string;
  target?: string;
  rel?: string;
  onClick?: () => void;
...
export default function Button({
  ...
  const classes = outline
    ? `group flex h-[3.6rem] items-center justify-between rounded-full border-[3px] border-candy-ruby pl-[27px] pr-[2.8px] font-display text-lg uppercase text-candy-ruby transition-transform hover:scale-[1.03] active:scale-[0.98] md:inline-flex md:h-[4.5rem] md:justify-center md:px-9 md:text-3xl ${fullWidth ? "w-full md:w-auto" : ""} ${className}`
    : `group inline-flex items-center gap-6 rounded-full font-display uppercase transition-transform duration-200 ease-out hover:scale-[1.03] active:scale-[0.98] ${fullWidth ? "w-full justify-between md:w-auto md:justify-start" : ""} ${v.pill} ${dims} ${className}`;

  if (href) {
    return (
      <a href={href} target={target} rel={rel} className={classes} onClick={rest.onClick}>
        {inner}
      </a>
    );
  }
